 ...
 * intraprocedural part can resolve the effect of method calls. This manipulates
 * PurityGraphBox.
 */
public class MyPurityIntraproceduralAnalysis extends ForwardFlowAnalysis {
		...
		// ///////////
			...
			if (leftOp instanceof Local) {
				...
				// v = v.f
					...
					Local right = (Local) ((InstanceFieldRef) rightOp)
							.getBase();
					String field = ((InstanceFieldRef) rightOp).getField()
							.getName();
	...
	public void drawAsOneDot(String prefix, String name) {
		...
		while (it.hasNext()) {
			...
			while (itt.hasNext()) {
				Object dst = itt.next();
				DotGraphEdge edge = dot.drawEdge("head" + node.get(src), "head"
						+ node.get(dst));
		...
		File f = new File(SourceLocator.v().getOutputDir(), prefix + name
	 ...
	 */
	public MyPurityIntraproceduralAnalysis(UnitGraph g) {
		super(g);
		// this.inter = inter;
		doAnalysis();
	}
